...
- **BPE tokenization RAM (fixed)**: tokenizing 8.7 GB corpus with gpt2 produces ~2.9B tokens (~88 GB as int32). Tokenization runs in a `multiprocessing.Process` child (`_tokenize_worker` in `data_spanish.py:30`) to prevent CPython's pymalloc from accumulating heap arenas. Parent process RSS stays flat (~5-10 GB). Do NOT use `open_memmap` — OS page cache balloons.
